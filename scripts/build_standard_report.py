...
# Every "before" and "after" below is lifted verbatim from the snapshot and from
# the live row. Nothing here is a paraphrase of what the page says.
FEATURED = [
    {
        "deed": "הדי לאמאר",
        ...
        "after": "הדי לאמאר ואנתייל רשמו פטנט להכוונת טורפדו בקפיצת תדרים, ולא קיבלו אגורה",
        "why": "תיק הפטנט מראה שהתביעה השביעית — היחידה שניסחה את עקרון קפיצת התדרים — נדחתה "
               "ב-13.8.1941 כמכוסה בפטנטים קודמים, ועורכי הדין מחקו אותה. ממציא הבלוטות׳ העיד שלא "
               "הכיר את הפטנט, וה-Wi-Fi זנח קפיצת תדרים מוקדם. מה שנשאר — פטנט אמיתי, רעיון מקורי, "
               "ואפס תמורה — חזק יותר מהכותרת שנמחקה.",
    ...
    },
    {
        ...
        "field": "איך זה התחיל",
        "before": "מבצע הגיוס הראשון היה ב-1996 עבור משה סחייק ז\"ל וגייס 5,000 תורמים; "
                  "שנתיים אחר כך קם המאגר.",
        "after": "5,000 איש נבדקו — ולא נמצאה אף התאמה. סחייק מת.",
        "why": "אתר עזר מציון עצמו כותב: “no match was found among the 5,000 people who participated "
               "in the drive. Sadly, the young man died”. הטקסט הקודם ספר את התורמים ודילג על הסוף. "
               "באותה הזדמנות נמחק מהדף גם מניע אישי שיוחס למייסדים — מחלה במשפחה ב-1991 — "
               "שלא נמצא באף אחד מששת המקורות שנמשכו.",
    ...
    },
    {
        ...
        "after": "צה\"ל פתח מחדש בית-חולים נטוש בקהרמאנמרש וטיפל ב-470 פצועי רעידת האדמה",
        "why": "שני כתבי-עת שפיטים כותבים במפורש “our personnel re-opened the hospital” ו-"
               ...
               "בית-חולים שדה ברמה 3, אך לא הקימה אותו — היא פתחה מחדש מוסד טורקי קיים בתוך 24 שעות. "
               "מאותו דף נמחק גם המשפט “רחוק מכל מצלמה”: המבצע תועד בהרחבה בידי דובר צה\"ל, "
               "סוכנות אנדולו הטורקית והעיתונות הבינלאומית.",
    ...
    },
    {
        ...
        "kicker": "מונח שהומצא",
        "field": "התיאור",
        "before": "ב-1982 גילה פרופסור דן שכטמן מהטכניון דפוס הפזה שסתר את כל חוקי הגבישנות המקובלים.",
        "after": "בבוקר 8 באפריל 1982 … הביט דן שכטמן במיקרוסקופ האלקטרוני וראה תבנית עקיפה בעלת "
                 "סימטריה של פי עשרה. ספרי הקריסטלוגרפיה קבעו שסימטריה כזו אסורה בגביש מחזורי.",
        "why": "“דפוס הפזה” אינו מונח מקצועי ואינו קיים באף מקור, ו”כל חוקי הגבישנות” הגזמה: "
               "מדובר באיסור אחד, ספציפי ומדיד. בדרך התברר שגם עמוד העובדות של NobelPrize.org שוגה "
               "ומדבר על קרני רנטגן — הגילוי נעשה במיקרוסקופ אלקטרוני, כפי שכתוב בהודעת הפרס "
               "של אותו גוף עצמו.",
...
def render(deeds, out):
    ...
    feat_html = "".join(f"""<article class="card">
      <div class="top"><span class="kick">{esc(f['kicker'])}</span><span class="who">{esc(f['deed'])}</span>
        <span class="who">{esc(f['field'])}</span></div>
      <div class="ba">
